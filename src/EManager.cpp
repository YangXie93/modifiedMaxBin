...
void EManager::run_EM(int run_time)
{
    ...
    */
        for (run = 0; run < run_time; run++)
        {
            ...
            // Maximization
            /* Replacing traditional thread with ThreadPool
            i = 0;
            while (i < seed_num)
            {
            ...
            */
                for (i = 0; i < seed_num; i++)
                {
                    thread_pool->enqueue(std::bind(&EManager::threadfunc_M, this, i));
                }
            thread_pool->waitFinished();
...
void EManager::threadfunc_E(long double abund, int k)
{
    long double sum;
    int j;
    sum = 0;
